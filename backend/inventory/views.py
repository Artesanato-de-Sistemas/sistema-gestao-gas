...
class ProductViewSet(SupabaseViewSet):
    ...
    def movements(self, request, pk=None):
        ...
        try:
            ...
            return Response(mv_res.data[0] if mv_res.data else mv_payload, status=status.HTTP_201_CREATED)
...
class InboundView(APIView):
    ...
    def post(self, request):
        ...
        try:
            total_amount = sum(float(i.get("quantity", 0)) * float(i.get("unit_cost", 0)) for i in items)
