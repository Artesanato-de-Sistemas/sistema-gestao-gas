...
class DriversDashboardView(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            orders_res = supabase.table('orders').select('id, delivery_driver_id, total_amount, status').gte('created_at', since_str).execute()
            ...
            for driver in drivers:
                ...
                gross_amount = sum(float(o.get('total_amount') or 0) for o in driver_orders if o.get('status') != 'CANCELADO')
...
class DashboardMetricsView(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            stock_p13 = sum((i.get('available_quantity', 0) or 0) for i in items if 'GLP_13KG_CHEIO' in (i.get('category') or ''))
            stock_p20 = sum((i.get('available_quantity', 0) or 0) for i in items if 'GLP_20KG_CHEIO' in (i.get('category') or ''))
            stock_p45 = sum((i.get('available_quantity', 0) or 0) for i in items if 'GLP_45KG_CHEIO' in (i.get('category') or ''))

            # Vendas de hoje
            orders_res = supabase.table('orders').select('total_amount, status').gte('created_at', today_start).execute()
